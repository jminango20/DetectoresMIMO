function [BER , SER] = EP_Beta(n_symbols,n_iterations,L_EP,bits_all,n_all,H_all,par)
...
n_errors = zeros(length(par.SNRdB_list),length(par.beta)); %Numero de Erros para cada SNR -> Simbolos
n_errors_bits = zeros(length(par.SNRdB_list),length(par.beta)); %Numero de Erros para cada SNR -> Bits
...
for loop1=1:n_iterations    
    ...
    for idx_beta = 1:length(par.beta)
        ...
        n_errors(ind_db,idx_beta)=err;
        n_errors_bits(ind_db,idx_beta)=err_bits;
        end %fim SNR
    end %fim beta
end
