function [BER , SER] = EPProva(n_symbols,n_iterations,L_EP,bits_all,n_all,H_all,par,beta)
...
for loop1=1:n_iterations    
    ...
    for ind_db=1:length(par.SNRdB_list)
        ...
        for loop2=1:n_symbols            
            ...
            VAR_qi = zeros(N,L_EP+1);
            NU_qi = zeros(N,L_EP+1);
            VAR_qi(:,1) = var_qi;
            NU_qi(:,1) = nu_qi;
            ...
            for l=1:L_EP                
                ...
                if L<=N
                    for k=1:L
                        cavity_i(:,k)=exp(-0.5*(A(k)-ti).^2./hi_2);
                    end
                else
                    for k=1:N
                        cavity_i(k,:)=exp(-0.5*(A-ti(k)).^2/hi_2(k));
                    end
                end
                ...
                %Novos medias e varianzas
                VAR_qi(:,l+1) = var_qi;
                NU_qi(:,l+1) = nu_qi;
                
                %if mean(VAR_qi(:,l+1)-VAR_qi(:,l))<10^(-10) && mean(NU_qi(:,l+1)-NU_qi(:,l))<10^(-10)
                %    break;
                %end
